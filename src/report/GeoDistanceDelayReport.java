 ...
 */
public class GeoDistanceDelayReport extends Report implements MessageListener, GeoMessageListener {
	...
	public void geoMessageTransferred(GeoMessage m, GeoDTNHost from, GeoDTNHost to,
		...
		report(m.getId(), getSimTime() - info.getTime(), m.getHops().size()-1);
	}
	...
	public void newGeoMessage(GeoMessage m) {
		...
		this.creationInfos.put( m.getId(), 
				new InfoTuple(getSimTime(), 
						m.getFrom().getLocation().clone(), null) );
	}
	 ...
	 * @param hopCount The amount of hops it took to deliver the message
	 */
	private void report(String id, double time,
			int hopCount) {
		write("Unknown " + format(time) + " " + hopCount + 
				" " + id);
	}
