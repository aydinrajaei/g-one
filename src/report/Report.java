 ...
 *
 * Modified: cooldown period has been implemented for geocast scenarios****
 *
 */
package report;
...
public abstract class Report {
	...
	/** Cool down period -setting id ({@value}). Defines how many seconds from
	 *  the ending of the simulation should not be included in the reports.
	 *  Implementation of the feature is report specific, so check out the
	 *  respective report classes for details. Default is 0. Must be a positive
	 *  integer or 0. */
	...
	protected Set<String> cooldownIDs;
	...
	protected boolean isCooldown() {
		return this.cooldownTime < SimClock.getTime();
	}
	...
	/**
	 * Adds a new ID to the cool down ID set
	 * @param id The ID
	 */
	protected void addCooldownID(String id) {
		if (this.cooldownIDs == null) { // lazy creation of the Set
			this.cooldownIDs = new HashSet<String>();
		}
		
		this.cooldownIDs.add(id);
	}
	...
	/**
	 * Removes a cool down ID from the cool down ID set
	 * @param id The ID to remove
	 */
	protected void removeCooldownID(String id) {
		this.cooldownIDs.remove(id);
	}
	...
	/**
	 * Returns true if the given ID is in the cool down ID set
	 * @param id The ID
	 * @return true if the given ID is in the cool down ID set
	 */
	protected boolean isCooldownID(String id) {
		if (this.cooldownIDs == null || this.cooldownIDs.size() == 0) {
			return false;
		}
		
		return this.cooldownIDs.contains(id);
	}
