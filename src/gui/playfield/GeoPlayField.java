 ...
 *
 */
public class GeoPlayField extends JPanel {
	...
	private CastGraphic castGraphic;
	private boolean showMapGraphic;
	private boolean showCastGraphic;
	 ...
	 */
	public GeoPlayField (GeoWorld w, GeoDTNSimGUI gui) {
		this.w = w;
		this.gui = gui;
	...
	/**
	 * Sets the source for the cast graphics and enables cast graphics showing
	 * @param simMap The map to show
	 */
	public void setCast(CastSim simcast) {
		this.castGraphic = new CastGraphic(simcast);
		this.showCastGraphic = true;
	}
	 ...
	 * @param show True if the map graphics should be shown (false if not)
	 */
	public void setShowCastGraphic(boolean show) {
		this.showCastGraphic = show;
	}
	...
	public void paint(Graphics g) {
		...
		// draw casts (is exists and drawing requested)
				if (castGraphic != null && showCastGraphic) {
					castGraphic.draw(g2);
				}
		
		// draw hosts
		for (DTNHost h : w.getHosts()) {
			new GeoNodeGraphic(h).draw(g2); 
		}
