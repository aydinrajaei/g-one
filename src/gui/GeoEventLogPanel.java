 ...
 */
public class GeoEventLogPanel extends JPanel 
	implements ConnectionListener, MessageListener, GeoMessageListener, ActionListener {
	...
	private static final String GMSG_PROP = "geomessage";
	...
	private EventLogControl gMsgCreateCheck;
	private EventLogControl gMsgTransferStartCheck;
	private EventLogControl gMsgRelayCheck;
	private EventLogControl gMsgRemoveCheck;
	private EventLogControl gMsgDeliveredCheck;
	private EventLogControl gMsgDropCheck;
	private EventLogControl gMsgAbortCheck;
	
	/**
	 * Creates a new log panel
	 * @param gui The where this log belongs to (for callbacks) 
	 */
	public GeoEventLogPanel(GeoDTNSimGUI gui) {
		this.gui = gui;
	...
	private EventLogControlPanel createControls() {
		...
		c.addHeading("geomessages");
		gMsgCreateCheck = c.addControl("created");
		gMsgTransferStartCheck = c.addControl("started relay");
		gMsgRelayCheck = c.addControl("relayed");
		gMsgDeliveredCheck = c.addControl("delivered");
		gMsgRemoveCheck = c.addControl("removed");
		gMsgDropCheck = c.addControl("dropped");
		gMsgAbortCheck = c.addControl("aborted");
		return c;
	}
	 ...
	 */
	private void addEvent(String description, DTNHost host1,
			DTNHost host2, GeoMessage geomessage, boolean highlight) {
		JPanel eventPane = new JPanel();
		eventPane.setLayout(new BoxLayout(eventPane,BoxLayout.LINE_AXIS));
		...
		if (geomessage != null) {
			addInfoButton(eventPane, geomessage, GMSG_PROP);
		}
	 ...
	 * @param message The message involved in the event (if any, can be null) 
	 */
	private void processGeoEvent(EventLogControl check, final String name,
			final DTNHost host1, final DTNHost host2, final GeoMessage geomessage) {
		String descString;	// String format description of the event
		...
		if (check.pauseOnEvent()) {
      	...
      	addEvent(name, host1, host2, geomessage, check.pauseOnEvent());	
	}

	
	// Implementations of ConnectionListener and MessageListener and GeoMessage Listener interfaces
	...
	public void geoMessageDeleted(GeoMessage m, GeoDTNHost where, boolean dropped) {
		if (!dropped) {
			processGeoEvent(gMsgRemoveCheck, "GeoMessage removed", where, null, m);
		}
		else {
			processGeoEvent(gMsgDropCheck, "GeoMessage dropped", where, null, m);
		}
	}
	...
	public void geoMessageTransferred(GeoMessage m, GeoDTNHost from, GeoDTNHost to,
		...
		else if (m.getTo().checkThePoint(to.getLocation())) { 
			processGeoEvent(gMsgDeliveredCheck, "GeoMessage delivered again", 
					from, to, m);
		}
		else {
			processGeoEvent(gMsgRelayCheck, "GeoMessage relayed", from, to, m);
		}
	}
	...
	public void newGeoMessage(GeoMessage m) {
		processGeoEvent(gMsgCreateCheck, "GeoMessage created", m.getFrom(), null, m);
	}
	...
	public void geoMessageTransferAborted(GeoMessage m, GeoDTNHost from, GeoDTNHost to) {
		processGeoEvent(gMsgAbortCheck, "GeoMessage relay aborted", from, to, m);
	}
	...
	public void geoMessageTransferStarted(GeoMessage m, GeoDTNHost from, GeoDTNHost to) {
		processGeoEvent(gMsgTransferStartCheck,"GeoMessage relay started", from,
				to,m);
	...
	public void actionPerformed(ActionEvent e) {
		...
		else if (source.getClientProperty(GMSG_PROP) != null) {
			// was a geo message button -> show information about the geo message
			GeoMessage m = (GeoMessage)source.getClientProperty(GMSG_PROP);
			gui.getInfoPanel().showInfo(m);
		}
	}
