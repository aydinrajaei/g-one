 ...
 * @author Ari
 */
public class GeoMessageTransferAcceptPolicy {
	...
	public GeoMessageTransferAcceptPolicy(Settings nsSettings) {
		Settings s;
	 ...
	 * was null
	 */
	private boolean checkSimplePolicy(Cast cast, Range [] policy, 
			Coord myCoord) {
		...
		for (Range r : policy) { 
			if (r.isInRange(TO_ME_VALUE) && cast.checkThePoint(myCoord)) {
				return true;
			}
			/* else if (r.isInRange(address)) {
				return true;
	 ...
	 */
	private boolean checkHopCountPolicy(GeoMessage m, ArithmeticCondition ac) {
		if (ac == null) {
			return true;
		} else {
	 ...
	 * @param m The geomessage to transfer
	 * @return True if the geomessage is OK to transfer, false is not
	 */
	public boolean acceptSending(DTNHost from, DTNHost to, Connection con,
		...
		Coord myCoord = from.getLocation();
		if (! (checkSimplePolicy(m.getTo(), this.toSendPolicy, myCoord) &&
	 ...
	 */
	public boolean acceptReceiving(DTNHost from, DTNHost to, GeoMessage m) {
		...
		Coord myCoord = from.getLocation();
		if (! (checkSimplePolicy(m.getTo(), this.toReceivePolicy,myCoord) && 
