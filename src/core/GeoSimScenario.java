 ...
 * Geo_One simulation run.
 */
public class GeoSimScenario implements Serializable {
	
	/** a way to get a hold of this... */	
	private static GeoSimScenario myinstance=null;
	...
	/** geo-router class -setting id ({@value})*/
	public static final String GEOROUTER_S = "georouter";
	...
	/** The world instance */
	private GeoWorld world;
	/** List of hosts in this simulation */
	protected List<GeoDTNHost> hosts;
	...
	/** Casts list for Geocasting scenarios */
	private CastSim CS;
	/** Cell list for Geocasting EvrRouter scenarios */
	private CastSim CL;
	...
	/** Global geomessage event listeners */
	private List<GeoMessageListener> geoMessageListeners;
	...
	protected GeoSimScenario() {
		...
		this.geoMessageListeners = new ArrayList<GeoMessageListener>();
		this.movementListeners = new ArrayList<MovementListener>();
		this.updateListeners = new ArrayList<UpdateListener>();
		this.appListeners = new ArrayList<ApplicationListener>();
		this.CS = new CastSim(); //Runs the Cast information
		this.CL = new CastSim(true); //implemented for Routers that need cell list
		...
		this.world = new GeoWorld(hosts, worldSizeX, worldSizeY, updateInterval, 
	 ...
	 */
	public static GeoSimScenario getInstance() {
		if (myinstance == null) {
			myinstance = new GeoSimScenario();
		}
		return myinstance;
	}
	...
	/**
	 * Returns the SimCast this scenario uses, or null if scenario doesn't
	 * use any cast
	 * @return SimCast or null if no cast is used
	 */
	public CastSim getCast() {
		return this.CS;
	}
	 ...
	 * @param ml The listener
	 */
	public void addGeoMessageListener(GeoMessageListener gml){
		this.geoMessageListeners.add(gml);
	}
	...
	protected void createHosts() {
		this.hosts = new ArrayList<GeoDTNHost>();
		...
		for (int i=1; i<=nrofGroups; i++) {
			...
			// creates prototypes of MessageRouter, GeoMessageRouter and MovementModel
			...
			GeoMessageRouter gmRouterProto = 
					(GeoMessageRouter)s.createIntializedObject(ROUTING_PACKAGE + 
							s.getSetting(GEOROUTER_S));
			...
			for (int j=1; j<=appCount; j++) {
				...
				try {
					...
					gmRouterProto.addApplication(protoApp);
			...
			for (int j=0; j<nrofHosts; j++) {
				...
				// prototypes are given to new GeoDTNHost which replicates
				// new instances of movement model and message router
				GeoDTNHost host = new GeoDTNHost(this.messageListeners, 
						this.movementListeners,	gid, interfaces, comBus, 
						mmProto, mRouterProto, gmRouterProto, this.geoMessageListeners);
				hosts.add(host);
			}
		}
	}
	 ...
	 */
	public List<GeoDTNHost> getHosts() {
		return this.hosts;
	}
	
	/**
	 * Returns the GeoWorld object of this scenario
	 * @return the GeoWorld object
	 */
	public GeoWorld getWorld() {
		return this.world;
	}

	/**
	 * Returns the CastSim object of this scenario
	 * @return the CastSim object
	 */
	public CastSim getCasts() {
		return CS;
	}
	
	/**
	 * Returns the cell list object of this scenario which is needed for the EvrRouter
	 * @return the CastSim object
	 */
	public CastSim getCells() {
		return CL;
	}
