...
public class CBRConnection extends Connection {
	...
	public int startTransfer(DTNHost from, Message m) {
		...
		assert this.gMsgOnFly == null : "Already transferring " + 
				this.gMsgOnFly + " from " + this.msgFromNode + " to " + 
				this.getOtherNode(this.msgFromNode) + ". Can't " + 
				"start transfer of " + m + " from " + from;
	...
	/**
	 * Sets a message that this connection is currently transferring. If geomessage
	 * passing is controlled by external events, this method is not needed
	 * (but then e.g. {@link #finalizeTransfer()} and 
	 * {@link #isMessageTransferred()} will not work either). Only a one geomessage
	 * at a time can be transferred using one connection.
	 * @param from The host sending the geomessage
	 * @param m The geomessage
	 * @return The value returned by 
	 * {@link MessageRouter#receiveMessage(Message, DTNHost)}
	 */
	public int startTransfer(DTNHost from, GeoMessage m) {
		...
		this.msgFromNode = from;
		GeoMessage newMessage = m.replicate();
		int retVal = ((GeoDTNHost) getOtherNode(from)).receiveGeoMessage(newMessage, (GeoDTNHost) from);

		if (retVal == GeoMessageRouter.RCV_OK) {
			this.gMsgOnFly = newMessage;
			this.transferDoneTime = SimClock.getTime() + 
			(1.0*m.getSize()) / this.speed;
		}
	...
	public void abortTransfer() {
		...
		if(msgOnFly != null){
			...
			((GeoDTNHost) getOtherNode(msgFromNode)).geoMessageAborted(this.gMsgOnFly.getId(),
					(GeoDTNHost) msgFromNode,getRemainingByteCount());
		}
	...
	public int getRemainingByteCount() {
		...
		if (msgOnFly == null && gMsgOnFly == null) {
			return 0;
		}
